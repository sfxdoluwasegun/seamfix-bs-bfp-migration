 ...
 *
 */
public class NullMsisdnAndSerialValidator extends AbstractSyncFileValidator implements IValidator {
	...
	public SyncItem validate(SyncItem syncItem) {
        ...
        for (MsisdnDetail msisdn : msisdns) {
        	...
        	if((msisdn.getMsisdn() == null || msisdn.getMsisdn().trim().isEmpty()) 
        			&& (msisdn.getSerial() == null || msisdn.getSerial().trim().isEmpty())){
        		
        		addBfpFailureLog(syncFile, msisdn, BfpRejectionReason.NULL_MSISDN_AND_SERIAL_NUMBER);
        ...
        if(msisdns.isEmpty()){
            syncFile.setValidationStatusEnum(ValidationStatusEnum.NULL_MSISDN_AND_SERIAL_NUMBER);
        } else {
            syncFile.setValidationStatusEnum(ValidationStatusEnum.VALID); 
        }
	...
	@Override
	public boolean skip() {
		return getBool(BfpProperty.SKIP_NULL_MSISDN_AND_SERIAL_VALIDATION);
	}
